...
public class PseudoRegularMeshBuilder : IMeshBuilder
{
                                    ...
                                    for (int y_ind = 0; y_ind < y_intervals[y_line]; ++y_ind)
                                    {
                                        for (int x_line = x0, x_interval_points = 0; x_line < x1; x_interval_points += x_intervals[x_line], ++x_line)
                                        {
                                            for (int x_ind = 0; x_ind < x_intervals[x_line]; ++x_ind)
                                            {
                                                ...
                                                Hexahedron hexagon = new(hexagon_vertex_numbers);
    ...
    private void FillBorder<VectorT>(string coordinate, List<VectorT> vertices, Dictionary<(int, int, int, int), (int vertex, string volume_material)> borderDictionary, int x, int y, int z, int n, double k, string volume_material) where VectorT : VectorBase<double, VectorT>
    {
        switch (dimension)
        {
            case Dimension.D2:
